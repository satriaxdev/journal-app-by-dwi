...
const selectCustomColor = document.getElementById("select-custom-color");
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!importedData.data || !Array.isArray(importedData.data)) {
                showErrorMessage('Format file tidak valid');
                return;
            }

            const isValidData = importedData.data.every(note => 
                note.id && note.title && note.content && note.mood && note.date && typeof note.pinned === 'boolean'
            );

            if (!isValidData) {
                showErrorMessage('Struktur data tidak valid');
                return;
            }

            localStorage.setItem('notes', JSON.stringify(importedData.data));
            hideModal(importExportModal);
            refreshNotes();
            showSuccessMessage('Catatan berhasil diimpor!');
            
        } catch (error) {
            showErrorMessage('File corrupt atau format salah');
        }
    };
...
saveDateBtn.onclick = () => {
    ...
    if (date && time) {
        ...
        showErrorMessage("Harap isi tanggal dan waktu!");
    }
};
...
selectCustomColor.onclick = () => {
    const color = hexColorInput.value;
    if (!isValidHexColor(color)) {
        showErrorMessage('Format warna HEX tidak valid!');
        return;
    }
    ...
    const darkerColor = color.replace(/^#/, '').match(/.{2}/g)
        .map(x => Math.max(0, parseInt(x, 16) - 30).toString(16).padStart(2, '0'))
        .join('');
    document.documentElement.style.setProperty('--primary-dark', `#${darkerColor}`);
    
    const lighterColor = color.replace(/^#/, '').match(/.{2}/g)
        .map(x => Math.min(255, parseInt(x, 16) + 60).toString(16).padStart(2, '0'))
        .join('');
...
document.querySelectorAll('.contact-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        if (type === 'email') {
            ...
            const message = "Hai tim pengembang! saya ingin memberi umpan balik tentang aplikasi Journal-App (tulis saran perbaikan/update disini) saya berterima kasih telah menciptakan aplikasi ini. Salam dari pengguna.";
...
window.onload = () => {
    ...
    if (savedTheme === 'custom') {
        const customColor = localStorage.getItem('customThemeColor') || '#2196F3';
        document.documentElement.style.setProperty('--primary-color', customColor);
        
        const darkerColor = customColor.replace(/^#/, '').match(/.{2}/g)
            .map(x => Math.max(0, parseInt(x, 16) - 30).toString(16).padStart(2, '0'))
            .join('');
        document.documentElement.style.setProperty('--primary-dark', `#${darkerColor}`);
        
        const lighterColor = customColor.replace(/^#/, '').match(/.{2}/g)
            .map(x => Math.min(255, parseInt(x, 16) + 60).toString(16).padStart(2, '0'))
            .join('');
        document.documentElement.style.setProperty('--primary-light', `#${lighterColor}`);
    } else {
